...
!  open(10, file="input_small.txt", status="old", iostat=stat)
  ...
  block
    ...
    do j = 1, max_j
      if (all(input(:,j) == '.')) expand_pos_j = [j, expand_pos_j]
    end do
    do i = 1, max_i
      if (all(input(i,:) == '.')) expand_pos_i = [i, expand_pos_i]
    end do
